...
class UserServices {
	static register = async (
		...
		try {
			...
			if (userExist) {
				return Response.error("Esté e-mail já está em uso!, Tente recuperar a senha", "CONFLICT");
			}
			...
			return Response.error(
				...
				"SERVER_ERR",
				{ errors: [{ field: "email", message: "Esse email já está em uso!" }] }
			);
		}
	};

	static authorize = async (email: string, password: string): Promise<IResponse> => {
		try {
